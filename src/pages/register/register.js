    ...
    const fetchChatRooms = async () => {
      try {
        const response = await axios.get(
          "https://letxchatapi.herokuapp.com/api/chatRooms"
        );
    ...
    if (passwordRef.current.value !== passwordConfirmRef.current.value) {
      ...
      try {
        const response = await axios.post(
          "https://letxchatapi.herokuapp.com/api/auth/register",
          newUser
        );
        // setIsValidUser(true);
        // setShowModal(true);
        ...
        await axios
          .all(
            subArray.map((member) =>
              axios.put(
                `https://letxchatapi.herokuapp.com/api/chatRooms/${member}/${response.data._id}`
              )
            )
          )

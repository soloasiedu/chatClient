...
const ChatPage = () => {
  ...
  useEffect(() => {
    const getCurrentChatRoomMessages = async () => {
      const response = await axios.get(
        `https://letxchatapi.herokuapp.com/api/messages/${currentChatRoom._id}`
      );
  ...
  useEffect(() => {
    const getChatRooms = async () => {
      try {
        const response = await axios.get(
          `https://letxchatapi.herokuapp.com/api/chatRooms/${user._id}`
        );
    ...
    if (newMessage) {
      ...
      try {
        const messageSent = await axios.post(
          "https://letxchatapi.herokuapp.com/api/messages",
          message
        );
    ...
    if (fileData && currentChatRoom) {
      ...
      try {
        const response = await axios.post(
          "https://letxchatapi.herokuapp.com/api/upload",
          data
        );
        const messageBody = {
          chatRoomId: currentChatRoom._id,
